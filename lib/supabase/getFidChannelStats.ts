...
async function getFidChannelStats(fid = '') {
  ...
  const { data, error } = await supabase
    ...
    .in('channel_id', CHANNELS.map(channel => channel.value))
  ...
  const accumulator = data.reduce((prev, curr) => {
    if (!prev[curr.channel_id]) prev[curr.channel_id] = {
      tipsCount: 0, tipsTotal: 0,
      receivedTipsCount: 0, receivedTipsTotal: 0
    };
