...
function getStakerScores(users: User[], channelId: string) {
  ...
  return Promise.all(
    users.map(async (user) => {
      ...
      const events: any = await getDaysEvents(event);
      const multiplier = events.reduceRight(
        (prev: any, event: any, index: any) => {
      ...
      if (days > 0) multiplier.score += multiplier.points * (1 + DAILY_APR) ** days;
