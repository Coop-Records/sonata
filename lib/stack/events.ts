...
export const eventStakeChannel = (id: string) => `channel_stake_${id}` as const;
export const eventStakeChannelFid = (id: string, fid: number) =>
  `channel_stake_${id}_${fid}` as const;
...
export const eventAirdropStaker = (id: string, fid: number) =>
  `weekly_channel_airdrop_${id}_${fid}` as const;
