...
const getResponse = async (req: NextRequest): Promise<NextResponse> => {
  ...
  if (tipperFid === recipientFid) {
    return NextResponse.json(
      { message: `Can not downvote yourself`, tipRemaining: 0, totalTipOnPost: 0 },
      { status: 400 },
    );
  }
  ...
  stack.track(`downvote_from_${tipperFid}`, { account: recipientWalletAddress, points: tipAmount });

  return NextResponse.json(
    {
      message: `Downvoted ${used_tip} NOTES`,
      usedTip: used_tip,
      tipRemaining: remaining_tip_allocation,
      totalTipOnPost: total_tip_on_post,
    },
...
async function callAllocateTip(
  ...
  const { data, error } = await supabase.rpc('downvote_tip_with_fid_recipient', {
    fid_input: fidInput,
